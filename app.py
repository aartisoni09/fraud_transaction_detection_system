...
Interactive dashboard for real-time fraud detection monitoring,
transaction scoring, case management, and analytics.
...
# ── Configuration (Issue #1 fix: no more hardcoded URL) ────────
API = os.environ.get("API_URL", "http://localhost:8000")


# ── API Readiness Check ───────────────────────────────────────
def wait_for_api(retries: int = 5) -> bool:
    """Wait for the API to be ready on cold start.
...
def api_get(path: str, params: dict = None) -> dict | None:
    ...
    try:
        ...
        r = requests.get(f"{API}{path}", params=params, headers=headers, timeout=6)
...
def api_post(path: str, data: dict = None) -> dict | None:
    ...
    Args:
        path: API endpoint path (e.g., '/score').
    ...
    try:
        ...
        if api_key:
            headers["X-API-Key"] = api_key
        r = requests.post(f"{API}{path}", json=data, headers=headers, timeout=10)
        return r.json() if r.ok else None
    except Exception:
        return None
...
def api_patch(path: str, data: dict) -> dict | None:
    ...
    Args:
        path: API endpoint path (e.g., '/cases/CASE-123').
        data: JSON payload with updates.
    ...
    try:
        ...
        if api_key:
            headers["X-API-Key"] = api_key
        r = requests.patch(f"{API}{path}", json=data, headers=headers, timeout=6)
        return r.json() if r.ok else None
    except Exception:
        return None
...
with st.sidebar:
    st.markdown(
        '<div class="brand">🛡️ Fraud<span>Guard</span></div>',
        unsafe_allow_html=True,
    )
    ...
    if health:
        ...
        st.caption("Start: uvicorn api:app --port 8000")

    st.markdown("---")

    # Issue #8 fix: load-dataset now uses POST
    if st.button("📥 Load Dataset (1500 txns)"):
        with st.spinner("Scoring all 1500 transactions…"):
            res = api_post("/transactions/load-dataset")
        if res:
...
if page == "📊 Dashboard":
    ...
    # Issue fix: moved data fetching inside the page block
    summary = api_get("/analytics/summary")
    txns_raw = api_get("/transactions/recent", {"limit": 1500})

    # Issue fix: corrected indentation — dashboard content now renders properly
    if not summary or "message" in summary or summary is None:
        st.info(
            "👈 Click **Load Dataset** in the sidebar to populate the dashboard."
        )
        st.stop()
    ...
    with col2:
        if txns_raw and txns_raw.get("transactions"):
            ...
            fig.add_vline(
                x=health.get("threshold", 0.05) if health else 0.05,
                line_dash="dash",
                line_color="#ef4444",
    ...
    if txns_raw and txns_raw.get("transactions"):
        ...
        with col3:
            if "location" in df_t.columns:
                loc_risk = (
                    df_t.groupby("location")["fraud_probability"]
                    .mean()
                    .reset_index()
        ...
        with col4:
            if "transaction_type" in df_t.columns:
                tt_risk = (
                    df_t.groupby("transaction_type")["fraud_probability"]
                    .mean()
                    .reset_index()
                )
                fig = px.bar(
    ...
    if txns_raw and txns_raw.get("transactions"):
        ...
        df_show["fraud_probability"] = df_show["fraud_probability"].apply(
            lambda x: f"{x:.2%}"
        )
        ...
        def color_risk(val):
            ...
            colors = {
                "CRITICAL": "background-color:#3d0000",
    ...
    with st.form("score_form"):
        ...
        with c1:
            ...
            txn_id = st.text_input(
                "Transaction ID", value=f"TXN-{int(time.time())}"
            )
            ...
            txn_type = st.selectbox(
                "Transaction Type", opts["transaction_types"]
            )
        ...
        with c2:
            ...
            is_new_user = st.radio(
                "New User?",
                [0, 1],
                format_func=lambda x: "Yes" if x else "No",
            )
            prev_txns = st.number_input(
        ...
        with c3:
            ...
            is_night = st.checkbox(
                "Night Transaction (auto from hour)",
                value=(hour >= 22 or hour < 6),
            )
        ...
        submitted = st.form_submit_button(
            "🚀 Score Now", use_container_width=True
        )
    ...
    if submitted:
        ...
        if not result:
            ...
            with col2:
                ...
                vals = [
                    ms.get("random_forest", 0),
                    ms.get("gradient_boosting", 0),
                    ms.get("logistic_regression", 0),
                    ms.get("ensemble", 0),
                ]
                fig = go.Figure(
                    go.Bar(
                ...
                fig.update_layout(
                    title="Model Scores", yaxis=dict(range=[0, 1.15])
                )
                st.plotly_chart(
                    dark_chart(fig, 270), use_container_width=True
                )
            ...
            if result.get("case_id"):
                st.warning(
                    f"📋 Case auto-created: **{result['case_id']}**"
                )
...
# ════════════════════════════════════════════════════════════
    ...
    with col1:
        status_f = st.selectbox("Status", ["all", "open", "resolved"])
    with col2:
        priority_f = st.selectbox("Priority", ["all", "urgent", "high"])
    with col3:
        st.write("")
    ...
    if not data or not data.get("cases"):
        st.info(
            "No cases yet. Load the dataset or score a high-risk transaction."
        )
        st.stop()
            ...
            with right:
                if case["status"] == "open":
                    analyst = st.text_input(
                        "Analyst ID",
                        "analyst_01",
                        key=f"a_{case['case_id']}",
                    )
                    decision = st.selectbox(
                        "Decision",
                        ["approve", "decline", "hold", "investigate"],
                        key=f"d_{case['case_id']}",
                    )
                    notes = st.text_area(
                        "Notes",
                        key=f"n_{case['case_id']}",
                        height=70,
                    )
                    if st.button(
                        "✅ Submit", key=f"s_{case['case_id']}"
                    ):
                        upd = api_patch(
                    ...
                    st.success(
                        f"✅ **{case.get('decision', '—').upper()}**"
                    )
                    st.markdown(
                        f"By: {case.get('analyst_id', '—')}"
                    )
                    if case.get("notes"):
                        st.info(case["notes"])
...
# ════════════════════════════════════════════════════════════
    ...
    with c1:
        st.metric("AUC-ROC", f"{perf['auc_roc']:.4f}")
    with c2:
        st.metric("Precision", f"{fraud_r.get('precision', 0):.4f}")
    with c3:
        st.metric("Recall", f"{fraud_r.get('recall', 0):.4f}")
    with c4:
        st.metric("F1-Score", f"{fraud_r.get('f1-score', 0):.4f}")
...
# ════════════════════════════════════════════════════════════
    ...
    for _, row in df_alerts.iterrows():
        ...
        em = RISK_EMOJI[risk]
        actual = (
            f" | **Actual label: {'🔴 FRAUD' if row.get('actual_class', 0) == 1 else '🟢 LEGIT'}**"
...
# ════════════════════════════════════════════════════════════
    ...
    if loc_f and "location" in df:
        mask = mask & df["location"].isin(loc_f)
    if dev_f and "device" in df:
        mask = mask & df["device"].isin(dev_f)
    df_f = df[mask].copy()
    ...
    with col2:
        if "time" in df_f.columns:
            hour_risk = (
                df_f.groupby("time")["fraud_probability"]
                .mean()
                .reset_index()
            )
            fig = px.line(
                ...
                y="fraud_probability",
                markers=True,
                title="Avg Fraud Probability by Hour",
            )
            fig.add_vline(
                x=22,
                line_dash="dash",
                line_color="#ef4444",
                annotation_text="Night starts",
            )
            st.plotly_chart(dark_chart(fig, 380), use_container_width=True)
    ...
    st.download_button(
        "⬇️ Download CSV", csv, "fraud_results.csv", "text/csv"
    )
